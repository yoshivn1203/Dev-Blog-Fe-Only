...
description: This blog explores AWS VPC, subnets, Internet Gateways, and
  security groups, highlighting their roles and differences
---
...
2. Update the subnet’s route table to direct traffic (e.g., `0.0.0.0/0`) to the IGW.
...
3. Enable DNS hostnames and DNS resolution (for RDS connectivity).
...
2. Attach it to `nestjs-vpc`.

3. Update the public subnet’s route table:

   - Create a route table (`public-rt`) and associate it with `public-subnet-1`.

   - Add a route: Destination `0.0.0.0/0` → Target `nestjs-igw`.
...
- **EC2 Security Group** (for NestJS app):
  ...
  - Inbound Rules:
    ...
    - Type: SSH, Port: 22, Source: `<your-ip>/32` (for admin access)
...
1. Launch an EC2 instance:
   ...
   - Security Group: `nestjs-sg`

   - Assign a public IP (auto-assign enabled).

2. SSH into the instance, install Node.js, and deploy your NestJS app:
...
1. Launch an RDS instance (e.g., MySQL):
   ...
   - Subnet Group: Include `private-subnet-1` (and another private subnet if multi-AZ).

   - Security Group: `rds-sg`

   - Publicly Accessible: No.

2. Note the RDS endpoint (e.g., `mydb.xxxx.us-east-1.rds.amazonaws.com`).

3. Configure your NestJS app (e.g., in `app.module.ts`) to connect to the RDS endpoint:
